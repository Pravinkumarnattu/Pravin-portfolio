...
Update that file and the whole site reflects the change — no need to touch components.
...
- Routes are code-split at the router boundary is not yet enabled — for a site this size a single bundle (~130KB gzipped) loads fast enough that splitting isn't necessary, but `React.lazy` can be added per-route if you expand the site significantly
