  ...
  return (
    <div className="min-h-screen pt-32 pb-24">
      <div className="container-px max-w-5xl mx-auto">
        <Link to="/" className="inline-flex items-center gap-2 text-sm font-mono text-ink-muted hover:text-emerald-glow transition-colors mb-8">
        ...
        >
          <div>
            <p className="font-mono text-xs text-emerald-glow mb-2">resume/pravin_kumar.pdf</p>
            <h1 className="font-display text-3xl sm:text-4xl font-bold text-ink">{profile.name}</h1>
            <p className="text-ink-muted mt-2">Full Stack MERN Developer · {profile.location}</p>
          </div>
          <a
            href="/resume.pdf"
            download
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-8">
          <motion.div
            ...
            style={{ height: '80vh' }}
          >
            <iframe src="/resume.pdf" title="Pravin Kumar M — Resume" className="w-full h-full" />
          </motion.div>
          ...
          >
            <p className="font-mono text-xs text-ink-faint mb-4 font-mono-caption">WHY IT PASSES ATS</p>
            <ul className="space-y-3">
              {resumeHighlights.map((h) => (
                <li key={h} className="flex items-start gap-2.5 text-sm text-ink-muted">
                  <FiCheckCircle className="text-emerald-glow mt-0.5 shrink-0" size={15} />
                  {h}
                </li>
              ))}
            </ul>
